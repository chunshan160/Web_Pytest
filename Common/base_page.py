...
from Common.dir_config import screen_shots
...
class BasePage:
    ...
    def wait_element_visible(self, locator, times=30, poll_frequency=0.5):
        ...
        try:
            ...
            # log.error(e)
            self.save_screenshot(f"等待元素{locator}可见失败")
            raise e
    ...
    def save_screenshot(self, name):
        ...
        file_Path = screen_shots + "/" + f"{time1}"
        if not os.path.exists(file_Path):
            os.makedirs(file_Path)
        file_name = file_Path + "/" + f"{time2}-{name}.png"
        BaseTest.driver.save_screenshot(file_name)
        log.info(f"截取网页成功，文件路径为为：{file_name}")
